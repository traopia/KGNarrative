...
def main(argv, arc):
    ...
    dev_file = 'generated_dataset/' +  experiment_name + '/' + method + '_validation.json'
    ...
    tokenize_data = dataset.map(process_data_to_model_inputs, batched = True, remove_columns=['story','Instances Knowledge Graph', 'Types Knowledge Graph', 'Subclass Knowledge Graph'])


    ### load model
    model = AutoModelForSeq2SeqLM.from_pretrained("allenai/led-base-16384", gradient_checkpointing=True, use_cache=False).to(device)
    #model = AutoModelForSeq2SeqLM.from_pretrained("allenai/longformer-base-4096", gradient_checkpointing=True, use_cache=False)
    batch_size = 4 #we dont use htis 
    #collator to create batches. It preprocess data with the given tokenizer
    collator = transformers.DataCollatorForSeq2Seq(tokenizer, model=model).to(device)
